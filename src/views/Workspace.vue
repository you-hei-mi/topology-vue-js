  ...
  data: function() {
    return {
      topologyConfigs: {
        ...
        menus: defalutMenus,
        // monaco: 'http://localhost:8080/js/monaco',
    ...
    onEvent(e) {
      switch (e.name) {
        ...
        case 'addMaterial':
          ...
          this.$router.push({
            path: '/',
            query: { component: new Date().getTime() }
          });
          break;
        case 'editMaterial':
          // 编辑“我的组件”
        ...
        case 'save':
          ...
          if(this.data.component){
            console.log('获取保存组件所需要的 componentData 数据')
            console.log(window.topology.toComponent())
            console.log(JSON.stringify(window.topology.toComponent()))
          }
          // 比如这里表示保存文件
          break;
        case 'addImageUrl':
